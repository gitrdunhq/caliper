...
@click.option(
    ...
    help=(
        ...
        "'caliper review' maintains for the current directory (user cache dir, "
        "or a legacy ./.caliper/code_graph.sqlite if present)."
    ),
)
def query(question: str, db_path: str | None) -> None:
    ...
    \b
      ...
      caliper query "show me dead code"
      caliper query "what depends on ReviewPipeline"
      caliper query "are there circular imports?"
    """
    from caliper.core.nl_query import TEMPLATES, query_code
    from caliper.plugins._runners.graph_builder import resolve_graph_db_path

    if db_path is None:
        candidates = [resolve_graph_db_path(Path.cwd()), Path(".caliper/graph.db")]
    ...
    if not db.exists():
        ...
        click.echo(
            "Run 'caliper review --repo-path .' first to build the code graph.",
            err=True,
        )
        sys.exit(1)
