"""``PartingSession`` — owns the re-part (git IO) for ``caliper part --serve``.

# tested-by: tests/unit/test_part_serve.py

Split out of ``part_serve.py`` (which keeps the HTTP transport) so neither
file grows past the repo's 500-line cap. ``write_override`` is the only
mutation — a deterministic, idempotent write-back into ``.caliper.yaml``,
validated through ``PartingConfig`` before it touches disk so a bad bucket
never corrupts the file. ``PartingSession`` holds the repo/base/head and owns
the re-part; ``dispatch`` (in ``part_routes.py``) is the pure request router
over a session.
"""
