"""Configuration module for caliper.

All configuration is loaded from environment variables with the CALIPER_ prefix.
...
class _CommaSeparatedEnvSource(EnvSettingsSource):
    ...
    def decode_complex_value(self, field_name: str, field: Any, value: str) -> Any:  # noqa: ANN401
        ...
        try:
            ...
            # validation (e.g. CALIPER_SUPPLY_CHAIN_DIFF_ECOSYSTEMS=pypi).
...
class CaliperSettings(BaseSettings):
    """Caliper configuration loaded from CALIPER_* env vars.
    ...
    semgrep_community_rules_dir: str | None = None
    ...
    # Detect-then-scribe (ADR-006): which finding scribes run after detection.
    # On-by-default scribes are cheap+deterministic; semgrep is opt-in (per-file
    # subprocess cost). The whole pass is fail-open and bounded by scribe_timeout.
    enabled_scribes: list[str] = Field(default=list(DEFAULT_SCRIBES))
    scribe_timeout: int = 30
    ...
    # settings and the "supply_chain_threat" scribe (opt-in via enabled_scribes).
