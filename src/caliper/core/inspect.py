"""The inspection adjudicator — pure Tier 2 filter over LLM claims.

# tested-by: tests/unit/test_inspect_adjudicator.py

``adjudicate()`` is the deterministic gate between the LLM (Tier 1) and a human:
...
Rules, applied in firing order (a claim must survive all six; drops are logged
with the rule that killed them):
  ...
  2. anchor          drop claims not on a real changed line (hunk line map)
  3. substantiation  a blocking claim with no Tier 0 evidence_ref is DOWNGRADED to
  ...
  6. dedup           collapse {file, normalized_line, category}, keep highest severity
"""
...
def bind_evidence(
    claims: list[Claim], tier0: list[GaugeFinding], cfg: InspectConfig
) -> list[Claim]:
    """Post-hoc evidence binding: link a claim to a Tier 0 finding by file, overlapping
...
def adjudicate(
    ...
    map (file -> changed new-side line numbers) from the stock producer.
    """
    ...
    parsed = bind_evidence(parsed, tier0, cfg)
    ...
    # Rule 6 — dedup on {file, normalized_line, category}; keep highest severity.
