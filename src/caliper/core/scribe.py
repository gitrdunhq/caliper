"""ScribeNote value object + the canonical enclosing-symbol resolver (ADR-006).
# tested-by: tests/unit/test_scribe.py

Detect-then-scribe: after a plugin detects a finding, caliper attaches deterministic
...
the ``ScribeNote`` value object, the ``ScribeContext``, and the single
``enclosing_symbol`` resolver that both the cpd runner (``plugins``) and the
``EnclosingSymbolScribe`` (``detectors``) reuse (one source of truth).

Everything here is pure (stdlib ``ast``/``re``) and deterministic; scribes built on
...
class ScribeNote(Contract):
    """Deterministic context attached to a finding (serialized into ``metadata['scribe']``)."""
...
@dataclass
class ScribeContext:
    """Inputs the scribe pass hands to each scribe (core types only)."""

    repo_path: str
    scribe_timeout: float = 30.0


def merge_scribe(finding, *, source: str, **fields):
    """Merge scribe *fields* into ``finding.metadata['scribe']`` (accumulating sources).

    The single helper every scribe uses to attach context, so the merge semantics live in
    one place. Handles both representations a finding takes in caliper: a frozen ``PluginFinding``
    (returned via ``model_copy``) and a raw ``dict`` (semgrep/blast-radius style, returned as a
    new dict). The ``scribe`` envelope is always a plain dict (JSON-safe).
    """
    is_dict = isinstance(finding, dict)
    existing = finding.get("metadata") if is_dict else getattr(finding, "metadata", {})
    metadata = dict(existing or {})
    scribe = dict(metadata.get("scribe") or {})
    sources = list(scribe.get("sources", []))
    if source not in sources:
        sources.append(source)
    scribe.update(fields)
    scribe["sources"] = sources
    metadata["scribe"] = scribe
    if is_dict:
