"""SupplyChainThreatScribe — advisory LLM narrative over a version-bump diff (ADR-006).
...
to ``metadata['scribe']['threat_analysis']`` and is **purely advisory** — it
...
@SCRIBES.register("supply_chain_threat")
class SupplyChainThreatScribe:
    ...
    def scribe(self, finding: PluginFinding, ctx: ScribeContext) -> PluginFinding:
        ...
        try:
            ...
            logger.warning("scribe.supply_chain_threat.failed")
            return finding
        if not narrative:
            return finding
        return merge_scribe(
