"""CodeGraphScribe — attaches blast radius + enclosing symbol from the code graph (ADR-006).
...
``blast-radius`` plugin builds; this scribe builds it **once per run** and caches it on
the instance (the scribe pass is sequential, so no locking is needed).
...
plugin — and the whole pass is time-bounded by ``ScribeContext.scribe_timeout``.
"""
...
_MAX_CALLERS = 25  # cap the attached blast radius so scribe stays bounded


@SCRIBES.register("code_graph")
class CodeGraphScribe:
    ...
    def _resolve_graph(self, repo_path: str) -> CodeGraph | None:
        ...
        try:
            ...
            logger.exception("scribe.code_graph.build_failed", repo=repo_path)
            return None
        return self._graph
