"""SemgrepScribe — attaches nearby static-analysis rule matches (detect-then-scribe, ADR-006).
...
location as ``metadata['scribe']['related']``, so a consumer sees "what else the static
...
_MAX_RELATED = 10  # cap the attached matches so scribe stays bounded
...
class SemgrepScribe:
    ...
    def scribe(self, finding: PluginFinding, ctx: ScribeContext) -> PluginFinding:
        ...
        if not related:
            return finding
        return merge_scribe(finding, source=self.name, related=related[:_MAX_RELATED])
