...
def default_graph_db_dir(repo_path: Path | str) -> Path:
    ...
    Defaults to ``$XDG_CACHE_HOME/caliper/graphs/<repo-hash>`` (falling back to
    ...
    return cache_root / "caliper" / "graphs" / digest
...
def resolve_graph_db_path(repo_path: Path | str, configured: str | None = None) -> Path:
    ...
    2. *configured* — an explicit path from repo config
       (``thresholds.blast-radius.graph_db`` in ``.caliper.yaml``);
       relative values are resolved against *repo_path*.
    3. A pre-existing legacy ``<repo>/.caliper/code_graph.sqlite`` (the pre-0.3
    ...
    """
    env_override = os.environ.get("CALIPER_GRAPH_DB")
    if env_override:
        return Path(env_override)
    if configured:
        p = Path(configured)
        return p if p.is_absolute() else Path(repo_path) / p
    legacy = Path(repo_path) / ".caliper" / _GRAPH_DB_FILENAME
    if legacy.exists():
        return legacy
...
class CodeGraph:
    ...
    def symbol_at(self, file_path: str | Path, line: int) -> dict | None:
        ...
        *line*, the innermost (largest start line) wins. The scribe layer
    ...
    def purge_deleted_files(self, existing_files: list[str]) -> int:
        ...
        symbols (gitrdunhq/caliper#385). existing_files is retained for
