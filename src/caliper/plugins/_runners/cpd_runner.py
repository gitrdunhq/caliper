"""PMD CPD subprocess runner (detect) + deterministic scribe (ADR-006).
...
Scribe (ADR-006): each clone group is annotated deterministically — the enclosing
...
from caliper.core.scribe import enclosing_symbol as _enclosing_symbol_core
...
# --- scribe (ADR-006): deterministic, fail-open -----------------------------------------------
...
def _scribe_clones(dupes: list[dict], repo_path: str) -> list[dict]:
...
def run_cpd(
    ...
    _scribe_clones(all_dupes, repo_path)
    return {
