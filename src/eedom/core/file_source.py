...
"""File-source adapters — one seam for "which files does eedom scan?".

Two strategies satisfy :class:`~eedom.core.ports.FileSourcePort`:
...
* :class:`WalkFileSource` (key ``"walk"``) — ``os.walk`` with eedom's own
  ignore rules. The fail-open fallback for non-git targets (read-only mounts,
  tarball extracts, ``dom ../somedir``).

Both then apply the eedom exclusion layer (:mod:`eedom.core.ignore`) on top, so
...
repo and falls back to walk otherwise, with an ``EEDOM_FILE_SOURCE`` override.
...
class WalkFileSource:
    """Enumerate files via ``os.walk`` + eedom ignore rules (non-git fallback)."""
...
class GitLsFilesSource:
    ...
    def list_files(self, root: Path, *, suffixes: tuple[str, ...] | None = None) -> list[Path]:
        ...
        for rel in result.stdout.split("\0"):
            ...
            if not _matches_suffix(rel, suffixes):
                continue
            if should_ignore(rel, patterns):  # eedom exclusions on top of .gitignore
                continue
            out.append(root / rel)
...
def select_file_source(
    ...
    *,
    ...
    ``EEDOM_FILE_SOURCE`` env var) forces ``"git"`` or ``"walk"``; the default
    ``"auto"`` probes for a usable git work tree and falls back to walk.
    """
    choice = (prefer or os.environ.get("EEDOM_FILE_SOURCE", "auto")).strip().lower()
