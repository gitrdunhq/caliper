...
* ``CodeGraphGroundingProvider`` ("codegraph") — reuses eedom's SQLite
...
from eedom.core.registries import GROUNDING_PROVIDERS
...
class CodeGraphGroundingProvider:
    """Serve grounding from eedom's SQLite ``CodeGraph``.
        ...
        if paths is None:
            ...
            try:
                for p in sorted(Path(root).rglob("*")):
                    ...
                    if any(
                        seg in p.parts
                        for seg in (".git", "__pycache__", "node_modules", ".venv", ".eedom")
                    ):
                        continue
                    if p.suffix not in {
                        ".py",
