...
| `caliper inspect` | Manual, developer-invoked per-part review of a `caliper part` cut list. Three tiers per part: Tier 0 deterministic gauges (existing analyzers scoped to the part, routed by bucket; fail-closed), Tier 1 advisory LLM claims behind a sealed swappable `LLMPort` (cached on part content; fail-soft), Tier 2 a pure deterministic adjudicator that filters claims by six rules (parse, scope, anchor, substantiation, category allow-list, floor, dedup) so no LLM output reaches a human except through it. Writes per-part + integration reports. `--no-llm` is fully deterministic. Advisory only: never gates, never enters the audit lake, LLM backends isolated in a dedicated INSPECT_BACKENDS registry. `--explain` re-prints a report. |
