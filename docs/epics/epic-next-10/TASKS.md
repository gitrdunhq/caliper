...
## task-001: Fix score/grade consistency and verdict wording in review_summary
...
## task-002: Renderer: plugin table filtering, path relativization, detector sections, ordering, truncation
...
## task-004: Extend TEST_PATTERNS to match *-tests/*_tests/*-test/*_test directory components
...
## task-005: Parse .caliperignore per-path rule-id scoping syntax into typed RuleScope
...
## task-006: Default semgrep severity floor excludes sub-medium findings from verdict/scores
...
## task-007: caliper init documents .caliperignore rule-scope and severity-floor knobs
...
- **Depends on**: task-005, task-006, task-004
...
## task-008: Deterministic direct/transitive dependency_kind classification in manifest_discovery
...
## task-009: osv-scanner findings carry file/line from source.path and dependency_kind metadata
...
## task-010: trivy findings carry dependency_kind metadata
...
## task-011: Dependency report section: group by advisory, show installed->fixed, and fix-first list
...
- **Files**: src/caliper/core/renderer.py, src/caliper/templates/comment.md.j2, tests/unit/test_renderer.py
- **Depends on**: task-009, task-010, task-002, task-001, task-008
...
## task-012: SARIF places dependency findings on manifest file/line
...
## task-013: Rename PyPI distribution to caliper-review, keep import package/console script as caliper
...
## task-014: Update release-please workflow and docs for the new PyPI distribution name
...
- **Depends on**: task-009, task-010, task-011, task-002
...
## task-016: Scan cache key incorporates live-DB scanner db_updated_at
...
## task-017: JSON report schema gains optional db_version/db_updated_at field
...
## task-018: Mark ADRs 001/002/003/004/008 Superseded with decommission-log pointers
...
## task-019: Add new ADRs documenting semgrep pinning, test-exclusion default, severity vocabulary, tag-then-drop decommissioning
...
## task-020: Test that every ADR has a valid Status and no Accepted ADR references a dead module path
...
## task-021: Move contract/e2e CI jobs into the test-image container instead of CALIPER_ALLOW_HOST_TESTS
...
## task-023: install-scanners installs into --bin-dir even when a same-named binary exists on PATH
...
## task-024: Create .caliper-baseline.yaml so dogfood verdict is not blocked, with expiry test
...
## task-025: dogfood.yml fails the job on blocked/incomplete verdict
