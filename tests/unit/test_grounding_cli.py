...
DPS-12 domains:
  Integrity (SAFETY): the command runs only when CALIPER_GROUNDING_ENABLED is set.
...
def test_gated_off_prints_message_and_exits_zero(monkeypatch) -> None:
    monkeypatch.delenv("CALIPER_GROUNDING_ENABLED", raising=False)
    runner = CliRunner()
    result = runner.invoke(cli, ["ground", "--files", "src/caliper/core/policy.py"])
    assert result.exit_code == 0
    assert "gated off" in result.output


def test_enabled_emits_bundle_to_stdout(monkeypatch) -> None:
    monkeypatch.setenv("CALIPER_GROUNDING_ENABLED", "1")
    monkeypatch.setenv("CALIPER_GROUNDING_PROVIDER", "ctags")
    runner = CliRunner()
    result = runner.invoke(cli, ["ground", "--files", "src/caliper/core/config.py"])
    assert result.exit_code == 0
...
def test_enabled_writes_out_files(monkeypatch, tmp_path) -> None:
    ...
    result = runner.invoke(
        cli,
        ["ground", "--files", "src/caliper/core/config.py", "--out", str(out)],
    )
    assert result.exit_code == 0
    assert out.exists()
