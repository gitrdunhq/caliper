...
"""Tests for caliper.data.solver — issue solver with model fallback."""
...
class TestBuildPrompt:
    def test_includes_issue_number_in_prompt(self) -> None:
        from caliper.data.solver import SolverTask, build_prompt
...
class TestCleanCode:
    def test_strips_markdown_fences(self) -> None:
        from caliper.data.solver import _clean_code
...
class TestLooksLikePython:
    def test_recognizes_valid_test_code(self) -> None:
        from caliper.data.solver import _looks_like_python
...
class TestSolverConfig:
    def test_defaults(self) -> None:
        from caliper.data.solver import SolverConfig
    ...
    def test_model_ladder_starts_with_dense(self) -> None:
        from caliper.data.solver import ModelTier, SolverConfig
...
class TestSolverTask:
    def test_rejects_zero_issue_number(self) -> None:
        from caliper.data.solver import SolverTask
...
class TestSolverResult:
    def test_failed_result_has_error(self) -> None:
        from caliper.data.solver import SolverResult, TaskStatus
...
class TestOpenRouterResponse:
    def test_parses_valid_response(self) -> None:
        from caliper.data.solver import OpenRouterResponse
...
class TestSanitizeCode:
    def test_flags_dangerous_patterns(self) -> None:
        from caliper.data.solver import _sanitize_code
    ...
    def test_truncates_oversized_output(self) -> None:
        from caliper.data.solver import _MAX_CODE_LENGTH, _sanitize_code
...
class TestExtractRateLimit:
    def test_returns_wait_when_remaining_low(self) -> None:
        from caliper.data.solver import _extract_rate_limit
    ...
    def test_caps_wait_at_max(self) -> None:
        from caliper.data.solver import _MAX_RATE_LIMIT_WAIT_S, _extract_rate_limit
...
class TestModelSpec:
    def test_rejects_empty_id(self) -> None:
        from caliper.data.solver import ModelSpec, ModelTier
...
class TestAtomicWrite:
    def test_writes_file_atomically(self, tmp_path: Path) -> None:
        from caliper.data.solver import _atomic_write
...
class TestSolve:
    ...
    def _make_config(self, **kwargs):
        from caliper.data.solver import ModelSpec, ModelTier, SolverConfig
    ...
    def test_success_on_first_model(self) -> None:
        from caliper.data.solver import TaskStatus, solve
    ...
    def test_timeout_retries(self) -> None:
        ...
        config = self._make_config(
            max_retries=2,
            model_ladder=[
                __import__("caliper.data.solver", fromlist=["ModelSpec"]).ModelSpec(
                    id="test/model", tier="dense"
                )
            ],
        )
        with patch("caliper.data.solver.time.sleep"):
...
class TestSolveBatch:
    ...
    def test_on_result_called_per_task(self, tmp_path: Path) -> None:
        from caliper.data.solver import solve_batch

        callbacks = []
        config = self._make_config(tmp_path)

        with patch("caliper.data.solver.solve") as mock_solve:
            from caliper.data.solver import SolverResult, TaskStatus
